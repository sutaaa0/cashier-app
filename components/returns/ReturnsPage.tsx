...
export function ReturnsPage() {
  ...
  const handleReturn = async () => {
    if (returnedItems.every((item) => item.kuantitas === 0)) {
      toast({
        title: "Error",
        description: "Pilih produk yang ingin Anda kembalikan",
        variant: "destructive",
      });
      return;
    }
    ...
    <div className="container mx-auto p-4 font-mono">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        <div className="space-y-8">
          ...
          <TransactionDetails penjualanId={penjualanId} setPenjualanId={setPenjualanId} fetchTransactionDetails={fetchTransactionDetails} transactionDetails={transactionDetails} />
        </div>
        <div className="space-y-8">
          <ReturnedItems items={returnedItems} setItems={setReturnedItems} setTotalReturn={setTotalReturn} />
          <ReplacementItems items={replacementItems} setItems={setReplacementItems} setTotalReplacement={setTotalReplacement} activePromotions={activePromotions} />
        </div>
      </div>

      <RefundSummary totalReturn={totalReturn} totalReplacement={totalReplacement} additionalPayment={additionalPayment} setAdditionalPayment={setAdditionalPayment} diskonPoin={transactionDetails?.diskonPoin} />

      <div className="mt-8 flex justify-end">
        <Button onClick={handleReturn} disabled={isLoading || (totalReplacement > totalReturn && additionalPayment < totalReplacement - totalReturn)} className="px-6 py-3 bg-[#FFD700] text-black font-bold">
          {isLoading ? "Memproses..." : "Proses Pengembalian"}
        </Button>
      </div>

      {showRefundReceipt && refundDetails && <RefundReceiptModal isOpen={showRefundReceipt} onClose={() => setShowRefundReceipt(false)} data={refundDetails} />}
    </div>
  );
}
