...
import { getStockItems, updateStockItem, addStockItem } from "@/server/actions"
...
export function StockManagement() {
    ...
    try {
      ...
      if (allSuccess) {
        setStock(stock.map(item => {
          ...
          return change !== 0 
            ? { ...item, currentStock: item.currentStock + change, lastUpdated: new Date().toISOString() }
            : item
        }))
  ...
  const handleAddItem = async (newItem: Omit<StockData, "id" | "lastUpdated">) => {
    try {
      ...
      toast({
        title: "Error",
        description: "An error occurred while adding new item",
        variant: "destructive",
      })
    }
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <AddStockItemModal isOpen={isAddModalOpen} onClose={() => setIsAddModalOpen(false)} onAddItem={handleAddItem} />
    </div>
  )
}