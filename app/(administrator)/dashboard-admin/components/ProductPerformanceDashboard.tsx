      ...
      <div className="relative group">
        ...
        <div className="relative bg-white border-4 border-black p-4 transform -rotate-2 shadow-[8px_8px_0px_0px_rgba(0,0,0,1)]">
          ...
          <div className="font-mono bg-black text-white p-2 transform rotate-1 mb-2">
            {formatRupiah(categorySummary.totalSales)}
          </div>
          <div className="font-mono bg-white text-black p-2 border-2 border-black transform -rotate-1">
            {categorySummary.salesContribution.toFixed(1)}% of total sales
          </div>
        </div>
      </div>
    );
        ...
        <div className="relative bg-white border-4 border-black p-4 transform -rotate-2 shadow-[8px_8px_0px_0px_rgba(0,0,0,1)]">
          ...
          {payload.map((entry, index) => (
            <div key={index} className="mb-2">
              ...
              <div 
                className="font-mono p-2 transform rotate-1 border-2 border-black text-white" 
                style={{ backgroundColor: COLORS[index % COLORS.length] }}
              >
...
const ProductPerformanceDashboard: React.FC<ProductPerformanceDashboardProps> = ({ report }) => {
  ...
  const handleDownloadExcel = () => {
    ...
    const productsData = report.data.map((product: Produk) => ({
      ...
      "Selling Price": formatRupiah(product.PriceSale || 0),
      "Cost per Product": formatRupiah(product.PriceCapital || 0),
      "Sold": product.quantitySold,
      "Total Sales": formatRupiah(product.totalSales),
      "Contribution": `${product.salesContribution.toFixed(1)}%`,
      "Margin": `${product.profitMargin.toFixed(1)}%`,
      "Total Cost": formatRupiah(product.totalSales - product.profitAmount),
    ...
    const categoryData = report.categorySummary.map((category: CategorySummary) => ({
      ...
      "Total Sales": formatRupiah(category.totalSales),
      "Quantity Sold": category.quantitySold,
      "Contribution": `${category.salesContribution.toFixed(1)}%`,
      "Product Count": category.productCount,
      "Products With No Sales": category.productCountWithNoSales,
      "Best Selling Product": category.bestSellingProduct,
      "Average Margin": `${category.avgProfitMargin.toFixed(1)}%`
    }));
    ...
    const summaryData = [{
      ...
      "Total Sales": formatRupiah(report.totalSales),
      "Total Profit": formatRupiah(report.totalProfit),
      "Total Cost": formatRupiah(report.totalSales - report.totalProfit),
      "Average Profit Margin": `${report.avgProfitMargin.toFixed(1)}%`,
      "Total Products Sold": report.totalProductsSold,
      "Product Count": report.data.length,
      "Products With No Sales": report.productsWithNoSales.length,
      "Category Count": report.categorySummary.length,
      "Best Selling Product": report.data[0]?.productName || "N/A",
      "Best Selling Category": report.categorySummary[0]?.categoryName || "N/A",
    }];
    ...
    XLSX.utils.book_append_sheet(wb, ws4, "Products With No Sales");
  ...
  const handleDownloadPDF = () => {
    ...
    const productsTableColumns = [
      ...
      { header: 'Product', dataKey: 'name', width: 55 },
      { header: 'Selling Price', dataKey: 'price', width: 28 },
      { header: 'Cost', dataKey: 'cost', width: 28 },
      { header: 'Sold', dataKey: 'sold', width: 20 },
      { header: 'Total Sales', dataKey: 'totalSales', width: 30 },
      { header: 'Contribution', dataKey: 'contrib', width: 20 },
    ...
    doc.autoTable({
      ...
      didParseCell: (data) => {
        ...
        if (productData && Array.isArray(productData) && productData[4] === 0) { // "Sold" column = 0
    ...
    doc.setFontSize(10);
    doc.setFont("helvetica", "bold");
    doc.text("Report Summary", margin, footerY);
    ...
    const summaryPoints = [
      `• Total Sales: ${formatRupiah(report.totalSales || 0)}`,
      `• Total Cost: ${formatRupiah(report.totalSales - report.totalProfit || 0)}`,
      `• Total Profit: ${formatRupiah(report.totalProfit || 0)}`,
      `• Average Margin: ${(report.avgProfitMargin || 0).toFixed(1)}%`,
      `• Total Products Sold: ${(report.totalProductsSold || 0).toLocaleString()}`,
      `• Product Count: ${report.data.length || 0}`,
      `• Products With No Sales: ${report.productsWithNoSales?.length || 0}`
    ];
  ...
  return (
    <div className="relative">
      ...
      <div className="flex border-b-4 border-black mb-6">
        ...
        <button
          className={`px-6 py-3 mr-2 font-black text-lg transform ${activeTab === 'products' ? 'bg-black text-white -rotate-2' : 'bg-white rotate-1'} border-4 border-black transition-all`}
              ...
              <div className="relative transition-all duration-300">
                ...
                <ResponsiveContainer width="100%" height={300}>
                  ...
                  >
                    ...
                    <Bar 
                      dataKey="prevPeriodSales" 
                      name="Previous Period Sales" 
                      radius={[0, 4, 4, 0]}
                    >
                      {report.data.slice(0, 5).map((entry, index) => (
                        <Cell 
                          key={`prevPeriodSales-${index}`}
                          fill={COLORS[1]}
                          stroke="#000"
                          strokeWidth={2}
                        />
              ...
              <div className="relative transition-all duration-300">
                ...
                <div className="flex items-center justify-between mb-6">
                  <div className="transform -rotate-2 bg-gradient-to-r from-yellow-300 to-yellow-400 border-4 border-black p-3 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">
                    <h2 className="text-xl font-black tracking-tighter">CATEGORY SALES</h2>
                  </div>
                </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-8">
              ...
              {report.salesGrowth && (
                <div className={`flex items-center mt-2 ${report.salesGrowth >= 0 ? "text-green-500" : "text-red-500"}`}>
                  ...
                  <span className="font-bold">
                    {Math.abs(report.salesGrowth).toFixed(1)}%
                  </span>
                </div>
              )}
            </div>
            ...
            <div className="bg-white border-4 border-black p-6 shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] transform -rotate-1">
              ...
              <p className="font-bold mt-2">
                Margin: {report.avgProfitMargin.toFixed(1)}%
              </p>
            </div>
            ...
            <div className="bg-white border-4 border-black p-6 shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] transform -rotate-1">
              <h3 className="text-xl font-black mb-4">No Sales</h3>
            ...
            <table className="min-w-full bg-white">
              <thead className="bg-black text-white">
                <tr>
                  ...
                  <th className="py-3 px-4 text-right">Cost per Product</th>
