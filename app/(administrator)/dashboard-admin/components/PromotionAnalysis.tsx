...
export function PromotionAnalysis() {
  ...
  const bestPromotion = promotionData.length > 0 ? promotionData.reduce((best, current) => (current.profit > best.profit ? current : best)) : null;
          ...
          <div className="relative bg-white border-4 border-black p-4 transform -rotate-2 shadow-[8px_8px_0px_0px_rgba(0,0,0,1)]">
            ...
            <div className="space-y-2">
              <div className="font-mono bg-black text-white p-2 transform rotate-1">
                ...
                <span className="ml-2 font-bold" style={{ color: COLORS[0] }}>
                  Pendapatan
                </span>
              </div>
              ...
              <div className="font-mono bg-black text-white p-2 transform rotate-1">
                {formatRupiah(profit)}
                <span className="ml-2 font-bold" style={{ color: COLORS[1] }}>
                  Profit
                </span>
              </div>
              ...
              <div className="font-mono bg-black text-white p-2 transform rotate-1">
                {transactions}
                <span className="ml-2 font-bold" style={{ color: COLORS[2] }}>
                  Transaksi
                </span>
              </div>
            </div>
  ...
  return (
    <div className="relative bg-white border-4 border-black p-6 transition-all duration-300 group md:col-span-2">
      ...
      <div className="relative transition-all duration-300">
        ...
        {/* Subtitle */}
        <div className="mb-6 font-mono text-sm transform -rotate-1 inline-block bg-black text-white px-4 py-2">Kinerja promosi berdasarkan pendapatan & transaksi</div>
        ...
        <div className="h-[400px] relative border-4 border-black bg-white p-4">
          <ResponsiveContainer width="100%" height="100%">
            ...
            >
              ...
              <XAxis dataKey="name" stroke="#000" strokeWidth={2} tick={{ fill: "#000", fontWeight: "bold" }} />
              <YAxis yAxisId="left" orientation="left" stroke="#000" strokeWidth={2} tick={{ fill: "#000", fontWeight: "bold" }} tickFormatter={(value) => `${value / 1000}K`} />
              <YAxis yAxisId="right" orientation="right" stroke="#000" strokeWidth={2} tick={{ fill: "#000", fontWeight: "bold" }} />
              <Tooltip content={<CustomTooltip />} />
              <Bar yAxisId="left" dataKey="revenue" name="Revenue" radius={[4, 4, 0, 0]}>
                {promotionData.map((entry, index) => (
                  <Cell key={`revenue-${index}`} fill={COLORS[0]} className={`transition-all duration-300 ${activeBar === index ? "opacity-100" : "opacity-80"}`} stroke="#000" strokeWidth={2} />
                ))}
              </Bar>
              <Bar yAxisId="left" dataKey="profit" name="Profit" radius={[4, 4, 0, 0]}>
                {promotionData.map((entry, index) => (
                  <Cell key={`profit-${index}`} fill={COLORS[1]} className={`transition-all duration-300 ${activeBar === index ? "opacity-100" : "opacity-80"}`} stroke="#000" strokeWidth={2} />
                ))}
              </Bar>
              <Bar yAxisId="right" dataKey="transactions" name="Transactions" radius={[4, 4, 0, 0]}>
                {promotionData.map((entry, index) => (
                  <Cell key={`transactions-${index}`} fill={COLORS[2]} className={`transition-all duration-300 ${activeBar === index ? "opacity-100" : "opacity-80"}`} stroke="#000" strokeWidth={2} />
                ))}
              </Bar>
            </BarChart>
          </ResponsiveContainer>
        ...
        <div className="mt-6 grid grid-cols-3 gap-4">
          <div className="relative group/item border-3 border-black p-3 bg-white hover:-translate-y-1 transition-all duration-300">
            <div className="flex flex-col">
              ...
              <span className="font-mono bg-purple-100 text-black px-2 py-1 border-2 border-black mt-2">{bestPromotion?.name || "-"}</span>
            </div>
          ...
          <div className="relative group/item border-3 border-black p-3 bg-white hover:-translate-y-1 transition-all duration-300">
            <div className="flex flex-col">
              ...
              <span className="font-mono bg-green-100 text-black px-2 py-1 border-2 border-black mt-2">{formatRupiah(totalRevenue)}</span>
            </div>
            <div className="absolute inset-0 border-2 border-black transform translate-x-1 translate-y-1 -z-10" />
          </div>
          ...
          <div className="relative group/item border-3 border-black p-3 bg-white hover:-translate-y-1 transition-all duration-300">
            <div className="flex flex-col">
              ...
              <span className="font-mono bg-blue-100 text-black px-2 py-1 border-2 border-black mt-2">{formatRupiah(totalProfit)}</span>
            </div>
            <div className="absolute inset-0 border-2 border-black transform translate-x-1 translate-y-1 -z-10" />
          </div>
        </div>
        ...
        <div className="mt-6 grid grid-cols-3 gap-4">
          {[
            ...
            <div key={`legend-${index}`} className="relative group/item border-3 border-black p-3 bg-white hover:-translate-y-1 transition-all duration-300">
              <div className="flex items-center gap-2">
                <div className="w-6 h-6 border-2 border-black transform rotate-45 transition-transform group-hover/item:rotate-0" style={{ backgroundColor: item.color }} />
