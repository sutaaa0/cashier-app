...
export function AddCategoryModal({ isOpen, onClose, onCategoryAdded }: AddCategoryModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!iconFile) {
      toast({
        title: "Error",
        description: "Silakan unggah ikon kategori",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      if (result.status === "Success") {
        ...
        toast({
          title: "Error",
          description: result.message || "Gagal menambahkan kategori",
          variant: "destructive",
        });
      }
    } catch (error) {
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white border-4 border-black p-8 shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] w-full max-w-md">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-3xl font-black transform -rotate-2">TAMBAHKAN KATEGORI BARU</h2>
          <button
            onClick={handleClose}
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div>
            <label htmlFor="icon" className="block mb-2 text-xl font-bold">
              Ikon Kategori <span className="text-red-500">*</span>
            </label>
            <input type="file" id="icon" accept="image/*" onChange={(e) => e.target.files && setIconFile(e.target.files[0])} ref={fileInputRef} className="hidden" />
            <button
              type="button"
            ...
            >
              <Upload size={24} />
              {iconFile ? "Ubah Ikon " : "Unggah Ikon"}
            </button>
            {iconFile ? <p className="mt-2 text-sm font-bold text-green-600">✓ {iconFile.name}</p> : <p className="mt-2 text-sm text-red-500">Kolom ini wajib diisi</p>}
          </div>
