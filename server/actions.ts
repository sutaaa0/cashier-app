...
import { DetailPenjualan, Pelanggan, Prisma, Produk } from "@prisma/client";
  ...
  try {
        ...
        <body>
          <div class="receipt">
            <div class="header">
              ...
              <p>Date: ${new Date(receiptData.transactionDate).toLocaleString()}</p>
            </div>
            <div class="details">
              ...
              <div class="items">
                ${receiptData.orderItems
                  .map(
                    (item) => `
                  <div class="item">
                    <span>${item.nama} x${item.kuantitas}</span>
            ...
            <div class="totals">
              <div class="item">
                <strong>Total:</strong>
                <strong>Rp ${receiptData.finalTotal.toLocaleString()}</strong>
              </div>
              <div class="item">
                <span>Amount Received:</span>
                <span>Rp ${receiptData.amountReceived.toLocaleString()}</span>
              </div>
              <div class="item">
                <span>Change:</span>
    ...
    const uploadResponse = await new Promise((resolve, reject) => {
      cloudinary.uploader.upload(
        ...
        {
          resource_type: "raw",
          public_id: `receipts/${transactionId}`,
          folder: "pos-receipts",
          format: "pdf",
        },
        (error, result) => {
    ...
    const receipt = await prisma.receipt.create({
      data: {
        transactionId,
        penjualanId: receiptData.penjualanId,
        customerName: receiptData.customerName,
        petugasId: receiptData.petugasId,
        total: receiptData.finalTotal,
        pdfUrl: (uploadResponse as any).secure_url,
      },
    });
...
export async function getReceiptById(transactionId: string) {
  try {
...
// Optional: Add function to delete receipt from Cloudinary if needed
export async function deleteReceiptFromCloudinary(transactionId: string) {
  try {
    await cloudinary.uploader.destroy(
      `pos-receipts/receipts/${transactionId}`,
      { resource_type: "raw" }
    );
    return true;
  } catch (error) {
